 ...
 */
function addHeaderToDocument(document) {
    ...
    const autoGenerateFilePath = config.get('autoGenerateFilePath');
    let filePathComment = '';
    ...
    const company = config.get('company') || 'Unknown Company';
    let line1 = config.get('line1') || '';
    let line2 = config.get('line2') || '';
    let line3 = config.get('line3') || '';
    const commentType = config.get('commentType') || 'multiline';
...
function activate(context) {
    ...
    watcher.onDidCreate(async (e) => {
        const config = vscode.workspace.getConfiguration('auto-header-est2023');
        const autoHeader = config.get('autoHeader');
        if (autoHeader) {
