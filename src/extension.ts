...
function addHeaderToDocument(document: vscode.TextDocument) {
  ...
  const license = config.get<string>("license");
  const githubLink = config.get<string>("githubLink");
  ...
  const customHeaderTemplate = config.get<string>("customHeaderTemplate");
  const headerLayout = config.get<string[]>("headerLayout", []);
  ...
  line1 = line1 ? formatLine(line1) : "";
  line2 = line2 ? formatLine(line2) : "";
  line3 = line3 ? formatLine(line3) : "";

  let headerLines: string[] = [];

  if (customHeaderTemplate) {
    headerLines = customHeaderTemplate.split("\n").map(formatLine).filter(line => line.trim() !== "");
  } else {
    headerLayout.forEach((section) => {
      switch (section) {
        case "author":
          if (author) {headerLines.push(`Author: ${author}`);}
          break;
        case "date":
          if (date) {headerLines.push(`Created: ${date}`);}
          break;
        case "company":
          if (company) {headerLines.push(`Company: ${company}`);}
          break;
        case "license":
          if (license) {headerLines.push(`License: ${license}`);}
          break;
        case "githubLink":
          if (githubLink) {headerLines.push(`GitHub: ${githubLink}`);}
          break;
        case "line1":
          if (line1) {headerLines.push(line1);}
          break;
        case "line2":
          if (line2) {headerLines.push(line2);}
          break;
        case "line3":
          if (line3) {headerLines.push(line3);}
          break;
      }
    });
  }

  headerLines = headerLines.filter(line => line.trim() !== "");

  let headerComment: string;
  if (commentType === "singleline") {
    headerComment =
      filePathComment +
      headerLines.map((line) => `${commentSyntax.single} ${line}`).join("\n");
  } else {
    headerLines = headerLines.map((line) => `* ${line}`);
    headerComment =
      filePathComment +
      `${commentSyntax.multiStart}\n${headerLines.join("\n")}\n${
